...
def inference(
    ...
    ind2organ_merged = json.load(
        open(os.path.join(organs_dir_merged_path, "ind2organ.json"))
    )
    organ2voxels_merged = json.load(
        open(os.path.join(organs_dir_merged_path, "organ2voxels.json"))
    )
    organ2center_merged = json.load(
        open(os.path.join(organs_dir_merged_path, "organ2center.json"))
    )
    ...
    with torch.no_grad():
        ...
        for sentences, attn_mask, organs_indices, _ in tqdm(test_loader):
            ...
            if random_organ_point:
                ...
                pred_points = [
                    organ2center_merged[ind2organ_merged[str(ind.item())]]
                    for ind in y_pred
                ]
...
def parse_args():
    ...
    parser.add_argument(
        "--organs_dir_merged_path",
        type=str,
        help="Path to the data organs directory with merged organs.",
    )
    parser.add_argument(
