...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--do_lower_case", action="store_true", help="Use a cased language model."
    )
    parser.add_argument(
        "--max_seq_length",
        default=128,
    ...
    parser.add_argument(
        "--output_dir", type=str, help="Directory where the models are saved."
    )
    ...
    if args.model not in list(MODELS_PRETRAINED) + list(MODELS_FINETUNED):
        raise AttributeError(
            "Model should be selected in the list: "
            + ", ".join(list(MODELS_PRETRAINED) + list(MODELS_FINETUNED))
        )
    ...
    if args.model in MODELS_FINETUNED.keys():  # Build the SentenceTransformer directly
        word_embedding_model = models.BERT(
            path, max_seq_length=args.max_seq_length, do_lower_case=args.do_lower_case
        )
